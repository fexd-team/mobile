...
// 1. 重命名 ~demos 目录为 __demos
const oldDemosDir = path.join(docsDir, '~demos')
const newDemosDir = path.join(docsDir, '__demos')
...
files.forEach((file) => {
  if (file.endsWith('.js')) {
    ...
    if (content.includes('~demos') || content.includes('%7Edemos')) {
      content = content.replace(/~demos/g, '__demos')
      content = content.replace(/%7Edemos/g, '__demos')
