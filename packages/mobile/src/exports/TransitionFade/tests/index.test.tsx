...
describe('TransitionFade（CSSTransition + exd-fade）', () => {
  beforeEach(() => {
  ...
  test('L1：in 为 true 时渲染子节点', () => {
    ...
    expect(getByText('淡入内容')).toBeInTheDocument()
  })

  test('L2：传给 CSSTransition 的 classNames 包含 exd-fade', () => {
    render(
      <TransitionFade in>
        <div>内容</div>
      </TransitionFade>,
    )
    expect(rtgClassNames()).toMatch(/\bexd-fade\b/)
  })

  test('L6：in 为 false 时子节点不挂载（mountOnEnter + unmountOnExit）', () => {
    const { queryByText } = render(
      <TransitionFade in={false}>
        <div>隐藏</div>
      </TransitionFade>,
    )
