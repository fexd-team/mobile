...
import { FormOptions, Form, FormWatchOptions } from './type'
...
export default function createForm(formOptions: FormOptions = {}): Form {
  ...
  const buildReactivityData = (sourceData: any) => {
    ...
    const watchValues = (listener: any, options?: FormWatchOptions) => {
    ...
    const watchValue = (key: any, listener: any, options?: FormWatchOptions) => {
      const debouncedListener = getEventListener(listener, (options || {}).debounce)
      if (!supportProxy()) {
  ...
  const triggerRelativeWatcher = (eventName: any, listener: any, options?: FormWatchOptions) => {
    const eventListener = getEventListener(listener, (options || {}).debounce)
    return eventBus.on(eventName, eventListener)
  }
  const watchRelatives = (listener: any, options?: FormWatchOptions) =>
    triggerRelativeWatcher(RELATIVE_CHANGE, listener, options)
  const watchRelative = (key: any, listener: any, options?: FormWatchOptions) =>
