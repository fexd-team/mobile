...
export default () => {
  ...
  return (
    <div className="gap-4">
      ...
      <DemoBlock title="BlockPicker 状态一览">
        <BlockPicker
          options={options}
          label="错误状态，聚焦试试"
        ...
        <BlockPicker
          ...
          error="XXX有点问题哦"
        />
        <BlockPicker options={options} placeholder={'警告状态'} labelType="warn" helper="辅助文本" />
        <BlockPicker options={options} placeholder={'成功状态'} labelType="success" helper="辅助文本" />
        <BlockPicker disabled label={'禁用状态'} helper="禁用了，点也点不着" />
      </DemoBlock>

      <DemoBlock title="BlockPicker 不可清除">
        <BlockPicker options={options} placeholder="不可清除的" clearable={false} />
      </DemoBlock>

      <DemoBlock title="BlockPicker 各部分、多个排列">
        <BlockPicker
          options={options}
          label="label-1"
