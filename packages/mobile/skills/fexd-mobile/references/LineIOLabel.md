---
name: LineIOLabel
description: 行内 IO 标签
---

# LineIOLabel 行内 IO 标签

在 `UnstyledIOLabel` 上固定 `theme` 为 `LineLabel` 的 IO 区域标签容器，用于与 `LineInput`、`LinePicker` 等行内控件配套。属于 IO 分层体系中 **Line\*** 主题变体。

```tsx
import { LineIOLabel } from '@fexd/mobile'
```
...
```tsx
import { LineIOLabel } from '@fexd/mobile'
;<LineIOLabel label="姓名" placeholder="请输入" type="info" helper="辅助说明">
  <input />
</LineIOLabel>
```

```tsx
<LineIOLabel label="标题" error="出错提示" disabled hideErrorWhenFocusing />
```

## Props

`LineIOLabelProps` 为 `Omit<UnstyledIOLabelProps, 'theme'>`（`packages/mobile/src/exports/LineIOLabel/type.tsx`）。
...
| type | `'warn' \| 'error' \| 'info' \| 'success'` | `info` | 否 | 状态样式（`LineIOLabel` `defaultProps`） |
...
`LineIOLabelStyleVars` / `DOC_LineIOLabelStyleVars`：

| 变量                                | 说明             | 默认      |
| ----------------------------------- | ---------------- | --------- |
| `@line-io-label-disabled-color`     | 禁用态文字颜色   | `#c2bcbe` |
| `@line-io-label-prefix-line-height` | 前缀图标行高     | `22px`    |
| `@line-io-label-prefix-font-size`   | 前缀图标字体大小 | `14px`    |
| `@line-io-label-prefix-color`       | 前缀图标颜色     | `#c2bcbe` |
...
<!--
Source:
- packages/mobile/src/exports/LineIOLabel/type.tsx
- packages/mobile/src/exports/LineIOLabel/index.tsx
- packages/mobile/src/exports/LineIOLabel/style.less
-->
