...
# LineDatePicker 行内日期选择

行内（下划线）布局的日期选择控件，在 `UnstyledIODatePicker` 上固定 `theme` 为 `LineLabel`，用于表单行内录入场景。属于 IO 分层体系中 **Line\*** 主题变体，语义与 `DatePicker` / `UnstyledIODatePicker` 一致，仅视觉与 `LineLabel` 对齐。

```tsx
import { LineDatePicker } from '@fexd/mobile'
```
...
```tsx
import { LineDatePicker } from '@fexd/mobile'
;<LineDatePicker label="出生日期" placeholder="请选择" format="YYYY-MM-DD" onChange={(v) => console.log(v)} />
```
...
`LineDatePickerProps` 定义于 `packages/mobile/src/exports/LineDatePicker/type.tsx`，为 `Omit<UnstyledIODatePickerProps, 'theme'>`。下表按类型来源归纳（与源码交叉合并一致；未列出字段仍可能来自 `JSXDivProps`）。
...
| 属性            | 类型                           | 默认值 | 必填 | 说明                                        |
| --------------- | ------------------------------ | ------ | ---- | ------------------------------------------- |
| classNamePrefix | `string`                       | -      | 否   | 类名前缀（封装默认 `exd-line-date-picker`） |
...
- 基础日期弹层：`DatePicker`
- 行内标签皮肤：`LineLabel`
- 同类行内变体：`LineTimePicker`、`LinePicker`

## 样式定制

可通过 Less 变量覆盖（定义见 `LineDatePickerStyleVars` / `DOC_LineDatePickerStyleVars`）：

| 变量                                | 说明           | 默认               |
| ----------------------------------- | -------------- | ------------------ |
| `@line-date-picker-disabled-color`  | 禁用态文字颜色 | `ant-color-gray-7` |
| `@line-date-picker-clear-color`     | 清除按钮颜色   | `ant-color-gray-5` |
| `@line-date-picker-arrow-font-size` | 箭头图标大小   | `18px`             |
| `@line-date-picker-arrow-color`     | 箭头图标颜色   | `ant-color-gray-6` |

## 注意事项

- 组件目录未提供 `index.zh.md` 与 `demos/`，用法以 `UnstyledIODatePicker` 与 `DatePicker` 为准。
- `theme` 由封装固定为 `LineLabel`，调用方勿再传入。

<!--
Source:
- packages/mobile/src/exports/LineDatePicker/type.tsx
- packages/mobile/src/exports/LineDatePicker/index.tsx
- packages/mobile/src/exports/LineDatePicker/style.less
-->
